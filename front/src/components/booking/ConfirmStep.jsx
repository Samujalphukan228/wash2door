    ...
    const handleConfirm = async () => {
        try {
            ...
            if (result && (result.bookingCode || result.bookingId || result._id)) {
                ...
                // Result exists but doesn't have expected fields
                console.warn('⚠️ Unexpected result structure:', result)
                // Still treat as success if we got something back
                if (result) {
                    setBookingResult(result)
                    setSuccess(true)
                } else {
            ...
            // Check if the error message indicates success (edge case)
            const errorMessage = err.message || 'Failed to create booking'
            
            // If error contains "already booked", it's a real error
            if (errorMessage.toLowerCase().includes('already booked') ||
                ...
                // Generic error - might be a false negative
                setError('Something went wrong. Please check My Bookings to verify if your booking was created.')
            } else {
                setError(errorMessage)
            }
        } finally {
            ...
            >
                ...
                >
                    <div className="space-y-4">
                        ...
                        <div className="flex justify-between">
                            <span className="text-[10px] tracking-[0.15em] uppercase text-gray-400">Option</span>
                            <span className="text-[12px] text-black">
                        ...
                        <div className="flex justify-between">
                            ...
                            <span className="text-[12px] text-black">
                                {new Date(bookingResult.bookingDate || data.bookingDate).toLocaleDateString('en-IN', {
                                    weekday: 'short',
                                    day: 'numeric',
                                    month: 'short',
                                    year: 'numeric'
                        ...
                        <div className="flex justify-between">
                            <span className="text-[10px] tracking-[0.15em] uppercase text-gray-400">Location</span>
                            <span className="text-[12px] text-black text-right max-w-[200px]">
    ...
    return (
        ...
        >
            ...
            <button
                onClick={onBack}
                disabled={loading}
                className="flex items-center gap-2 text-[10px] tracking-[0.2em] uppercase text-gray-400 hover:text-black transition-colors duration-300 mb-8 disabled:opacity-50"
            >
                ...
                {error && (
                    ...
                    >
                        ...
                        <div>
                            ...
                            <button
                                onClick={() => router.push('/my-bookings')}
                                className="text-[10px] tracking-[0.1em] text-red-700 underline mt-2"
                            >
                                Check My Bookings →
                            </button>
                        </div>
            ...
            <div className="max-w-2xl mx-auto">
                    ...
                    <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-[11px] text-gray-500 mt-2">
                                {data._ui?.variantName || 'Option'} · {data._ui?.duration || 0} mins
                            </p>
                        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div className="border border-gray-200 p-6 rounded-[5px]">
                        ...
                        <p className="text-[14px] text-black" style={{ fontFamily: 'Georgia, serif' }}>
                            {data.bookingDate && new Date(data.bookingDate).toLocaleDateString('en-IN', {
                                weekday: 'long',
                                day: 'numeric',
                                month: 'long',
                                year: 'numeric'
                    ...
                    <div className="border border-gray-200 p-6 rounded-[5px]">
                        ...
                        <p className="text-[12px] text-black leading-5">
                            ...
                            {data.location?.landmark && (
                                <span className="text-gray-500"> (Near {data.location.landmark})</span>
                            )}
                        </p>
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                    <motion.button
                        ...
                        onClick={onBack}
                        disabled={loading}
                        className="px-8 py-4 border border-gray-200 text-[10px] tracking-[0.22em] uppercase hover:border-black transition-colors duration-300 rounded-[3px] disabled:opacity-50"
                    >
                        Edit Details
                    </motion.button>
                    <motion.button
                        ...
                        disabled={loading}
                        className="relative flex items-center justify-center gap-2 px-12 py-4 bg-black text-white text-[10px] tracking-[0.22em] uppercase overflow-hidden group disabled:opacity-70 rounded-[3px] min-w-[200px]"
                    >
                        {!loading && (
